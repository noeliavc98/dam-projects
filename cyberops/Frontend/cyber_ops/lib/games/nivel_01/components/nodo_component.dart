...
class NodoComponent extends CircleComponent with HasGameRef, CollisionCallbacks {
  final int id;
  NodoEstado estado;
...
void _renderCandado(Canvas canvas) {
  ...
  final cuerpo = RRect.fromRectAndRadius(
    Rect.fromCenter(center: Offset(cx, cy + 3), width: cuerpoW, height: cuerpoH),
  ...
  canvas.drawArc(
    Rect.fromCenter(center: Offset(cx, cy - 1), width: arcoR * 2, height: arcoR * 2),
    math.pi,
    math.pi,
    false,
    Paint()
...
void _renderDefensa(Canvas canvas) {
  ...
  canvas.drawRRect(
    baseRect,
    Paint()..color = _colorPorEstado().withValues(alpha: 0.9),
  );
  ...
  canvas.drawCircle(
    ...
    7,
    Paint()..color = borde.withValues(alpha: 0.9),
  );
  ...
  tp.paint(
    canvas,
    Offset(radio - tp.width / 2, radio - 4 - tp.height / 2),
  );
  ...
  idPainter.paint(
    canvas,
    Offset(radio - idPainter.width / 2, radio + 24),
  );
  ...
  switch (estado) {
    case NodoEstado.seguro:
      return const Color(0xFF6C63FF).withValues(alpha: 0.7);
    case NodoEstado.vulnerable:
